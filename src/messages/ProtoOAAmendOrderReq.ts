import {
  IProtoOAAmendOrderReq as IMessage,
  ProtoOAAmendOrderReq as Message,
  IProtoMessage,
  ProtoOAPayloadType
} from "../spotware-messages";
import SpotwareEventEmitter from "../spotware-event-emitter";

export namespace ProtoOAAmendOrderReq {
  export const payloadType = ProtoOAPayloadType.PROTO_OA_AMEND_ORDER_REQ;
  ...
  export function fromProtoMessage(
    ...
    throw new Error(
      `invalid message. expected payloadType 'PROTO_OA_AMEND_ORDER_REQ' (got '${
  ...
  export function emitDecoded(
    ...
    emitter.emit("PROTO_OA_AMEND_ORDER_REQ", data.message, data.clientMsgId);
  }
}
export default ProtoOAAmendOrderReq;
