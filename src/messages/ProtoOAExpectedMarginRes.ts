import {
  IProtoOAExpectedMarginRes as IMessage,
  ProtoOAExpectedMarginRes as Message,
  IProtoMessage,
  ProtoOAPayloadType
} from "../spotware-messages";
import SpotwareEventEmitter from "../spotware-event-emitter";

export namespace ProtoOAExpectedMarginRes {
  export const payloadType = ProtoOAPayloadType.PROTO_OA_EXPECTED_MARGIN_RES;
  ...
  export function fromProtoMessage(
    ...
    throw new Error(
      `invalid message. expected payloadType 'PROTO_OA_EXPECTED_MARGIN_RES' (got '${
...
export default ProtoOAExpectedMarginRes;
