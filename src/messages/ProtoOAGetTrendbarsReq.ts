import {
  IProtoOAGetTrendbarsReq as IMessage,
  ProtoOAGetTrendbarsReq as Message,
  IProtoMessage,
  ProtoOAPayloadType
} from "../spotware-messages";
import SpotwareEventEmitter from "../spotware-event-emitter";

export namespace ProtoOAGetTrendbarsReq {
  export const payloadType = ProtoOAPayloadType.PROTO_OA_GET_TRENDBARS_REQ;
  ...
  export function fromProtoMessage(
    ...
    throw new Error(
      `invalid message. expected payloadType 'PROTO_OA_GET_TRENDBARS_REQ' (got '${
  ...
  export function emitDecoded(
    ...
    emitter.emit("PROTO_OA_GET_TRENDBARS_REQ", data.message, data.clientMsgId);
  }
}
export default ProtoOAGetTrendbarsReq;
