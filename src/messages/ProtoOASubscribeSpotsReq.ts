import {
  IProtoOASubscribeSpotsReq as IMessage,
  ProtoOASubscribeSpotsReq as Message,
  IProtoMessage,
  ProtoOAPayloadType
} from "../spotware-messages";

export namespace ProtoOASubscribeSpotsReq {
  export const payloadType = ProtoOAPayloadType.PROTO_OA_SUBSCRIBE_SPOTS_REQ;
  ...
  export function fromProtoMessage(
    ...
    throw new Error(
      `invalid message. expected payloadType 'PROTO_OA_SUBSCRIBE_SPOTS_REQ' (got '${
        ProtoOAPayloadType[payloadType]
      }').`
    );
  }
}
export default ProtoOASubscribeSpotsReq;
