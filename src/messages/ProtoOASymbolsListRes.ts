import {
  IProtoOASymbolsListRes as IMessage,
  ProtoOASymbolsListRes as Message,
  IProtoMessage,
  ProtoOAPayloadType
} from "../spotware-messages";
import SpotwareEventEmitter from "../spotware-event-emitter";

export namespace ProtoOASymbolsListRes {
  export const payloadType = ProtoOAPayloadType.PROTO_OA_SYMBOLS_LIST_RES;
  ...
  export function fromProtoMessage(
    ...
    throw new Error(
      `invalid message. expected payloadType 'PROTO_OA_SYMBOLS_LIST_RES' (got '${
  ...
  export function emitDecoded(
    ...
    emitter.emit("PROTO_OA_SYMBOLS_LIST_RES", data.message, data.clientMsgId);
  }
}
export default ProtoOASymbolsListRes;
