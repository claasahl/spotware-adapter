import {
  IProtoOAAccountAuthReq as IMessage,
  ProtoOAAccountAuthReq as Message,
  IProtoMessage,
  ProtoOAPayloadType
} from "../spotware-messages";
import SpotwareEventEmitter from "../spotware-event-emitter";

export namespace ProtoOAAccountAuthReq {
  export const payloadType = ProtoOAPayloadType.PROTO_OA_ACCOUNT_AUTH_REQ;
  ...
  export function fromProtoMessage(
    ...
    throw new Error(
      `invalid message. expected payloadType 'PROTO_OA_ACCOUNT_AUTH_REQ' (got '${
  ...
  export function emitDecoded(
    ...
    emitter.emit("PROTO_OA_ACCOUNT_AUTH_REQ", data.message, data.clientMsgId);
  }
}
export default ProtoOAAccountAuthReq;
