...
import {
  ProtoMessage,
  ProtoOaPayloadType,
  ProtoOaOrderListByPositionIdResUtils,
...
export function deserialize(message: ProtoMessage): Type | undefined {
  if (
    ...
    return {
      payloadType: ProtoOaPayloadType.PROTO_OA_ORDER_LIST_BY_POSITION_ID_RES,
      payload: ProtoOaOrderListByPositionIdResUtils.read(pbf),
...
export function serialize(message: Messages): ProtoMessage | undefined {
  if (
    ...
    const pbf = new Pbf();
    ProtoOaOrderListByPositionIdResUtils.write(message.payload, pbf);
    return {
      ...message,
      payload: pbf.finish(),
    };
